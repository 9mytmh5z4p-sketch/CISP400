(function () {
  ...
  const documentElement = document.getElementById("homework-document");
  const SOURCE_PATH = documentElement.dataset.sourcePath || "./Toolset%20Test.md";
  const SOURCE_NAME = documentElement.dataset.sourceName || "Toolset Test.md";
  const ASSIGNMENT_SUFFIX = documentElement.dataset.assignmentSuffix || " Assignment";
  ...
  const allowedTags = new Set([
    ...
    "H4", "H5", "H6", "HR", "IMG", "KBD", "LI", "MARK", "OL", "P",
    "PRE", "S", "STRONG", "SUB", "SUMMARY", "SUP", "TABLE", "TBODY",
    "TD", "TFOOT", "TH", "THEAD", "TR", "UL"
  ]);

  const forbiddenTags = new Set([
    "BASE", "BUTTON", "CANVAS", "EMBED", "FORM", "FRAME", "FRAMESET", "IFRAME",
    "INPUT", "LINK", "MATH", "META", "NOSCRIPT", "OBJECT", "OPTION", "SCRIPT",
    "SELECT", "SOURCE", "STYLE", "SVG", "TEMPLATE", "TEXTAREA", "VIDEO", "AUDIO"
  ]);
  ...
  function addHeadingLinks() {
    ...
    for (const heading of documentElement.querySelectorAll("h1, h2, h3, h4, h5, h6")) {
      const base = heading.textContent.normalize("NFKD").toLowerCase()
  ...
  function renderMarkdown(markdown, label) {
    ...
    const html = globalThis.marked.parse(markdown, { gfm: true, breaks: false });
    documentElement.replaceChildren(sanitizeHtml(html));
    addHeadingLinks();
    documentElement.setAttribute("aria-busy", "false");
    const title = documentElement.querySelector("h1");
    if (title) document.title = title.textContent + ASSIGNMENT_SUFFIX;
  ...
  async function loadFromServer() {
    ...
    try {
      ...
      showMessage("The Markdown file could not be loaded", [
        "Use a local web preview or choose Open Markdown above and select " + SOURCE_NAME + ".",
  ...
  reloadButton.addEventListener("click", function () {
    if (location.protocol === "file:") fileInput.click();
    else loadFromServer();
  });
  ...
  if (location.protocol === "file:") {
    showMessage("Choose the Markdown assignment", [
      "Browsers do not automatically read a neighboring Markdown file when an HTML file is opened directly.",
      "Choose Open Markdown above and select " + SOURCE_NAME + ", or use a local web preview for automatic loading and refreshes."
    ], false);
